...
public interface BloodResultRepository extends JpaRepository<BloodResult, Integer> {
    ...
    @Query(value = "Select * from blood_result where user_id=:userId order by date", nativeQuery = true)
    List<BloodResult> findBloodResultByUserId(@Param("userId") int userId);

    @Query(value = "Select * from blood_result where parameter=:parameter and user_id=:userId order by date", nativeQuery = true)
